...
\end{equation}
Es wurde eine Luftfeuchtigkeit gemessen von $\varphi = 38 \ \%$. Aus dem Skript kann dadurch mit einer Gaskonstante von $R_S = 287,05 \ \frac{J}{kgK}$ gerechnet werden. 
\begin{equation*}
    \varrho = \frac{95545 \ Pa}{287,05 \ \frac{J}{kgK} \cdot 292,75 \ K} = 1,137 \;\frac{kg}{m^3}
\end{equation*}

Berechnung des Volumenstrom bei einem offen Rohr $D_D=200\ mm$ und einer Drehzahl von $n=2500 \ \frac{1}{min}$:
\begin{equation}
...
Berechnung des Drucks an Position 2:
\begin{equation}
    p_2=p_5 + \zeta \frac{\varrho}{2} c_5^2, \quad \text{wobei: } \zeta = 0,25
\end{equation}
\begin{equation*}
    p_2=40 \ Pa + 0,25 \cdot \frac{1,137 \ \frac{kg}{m^3}}{2} \cdot (13,85 \ \frac{m}{s})^2 = 67,26 \ Pa
\end{equation*}

mit $c_5$ der mittleren Strömungsgeschwindigkeit im Rohr
...
\begin{equation}
    \Delta p_{t,12} = 1,25 \cdot \frac{\rho}{2} \cdot c_5^2 + p_5
\end{equation}

\begin{equation*}
    \Delta p_{t,12}= 1,25 \cdot \frac{1,137 \ \frac{kg}{m^3}}{2} \cdot (13,85 \ \frac{m}{s})^2 + 40 \ Pa = 176,31 \ Pa
\end{equation*}
...
Berechnung der dimensionslosen Kennzahlen: 
\newline
Lieferzahl $\phi$ : 
\begin{equation}
    \phi = \frac{\dot V}{\frac{\pi^2}{4}\cdot D_L^3 \cdot n}
\end{equation}

\begin{equation*}
    \phi = \frac{0,435 \ \frac{m^3}{s}}{\frac{\pi^2}{4}\cdot (0,5\ m)^3 \cdot (\frac{2500}{60}\frac{1}{s})} = 0,0338
\end{equation*}
\newline
